...
import { getActiveTasks, getTask, createSubmission, updateSubmission, getSubmission } from '../../db/tasks';
...
export function registerTaskHandlers(bot: Bot) {
  ...
  bot.callbackQuery('tasks:list', async (ctx) => {
    ...
    await ctx.reply('📋 *Твои задания на сегодня:*', { parse_mode: 'Markdown', reply_markup: keyboard });
  });
  ...
  bot.callbackQuery(/^tasks:view:(.+)$/, async (ctx) => {
    ...
    const keyboard = new InlineKeyboard().text('✅ Выполнил!', `tasks:done:${task.id}`);
  ...
  bot.callbackQuery(/^tasks:done:(.+)$/, async (ctx) => {
    ...
    await ctx.reply('📸 Отлично! Пришли фото выполнения, и я отправлю его родителям на проверку.');
  });
  ...
  bot.on('message:photo', async (ctx) => {
    ...
    const submission = await createSubmission({ taskId, taskTitle: task.title, childId: userId, photoFileId, status: 'pending' });

    await ctx.reply('✅ Отправил! Жди, пока родители проверят.');
  ...
  bot.callbackQuery(/^review:approve:(.+)$/, async (ctx) => {
    ...
    if (task?.type === 'once') {
      const { deactivateTask } = await import('../../db/tasks');
  ...
  bot.callbackQuery(/^review:reject:(.+)$/, async (ctx) => {
    ...
    await ctx.reply('✏️ Напиши комментарий для Макса — что нужно переделать:');
  });
  ...
  bot.on('message:text', async (ctx, next) => {
    ...
    if (pendingReject.has(userId)) {
      ...
      await ctx.reply('✅ Отправил комментарий Максу.');
      await ctx.api.sendMessage(
        submission.childId,
  ...
  bot.callbackQuery('admin:tasks:new', async (ctx) => {
    ...
    await ctx.reply('📝 *Создание задания*\n\nКак называется задание?', { parse_mode: 'Markdown' });
  });
  ...
  async function handleTaskCreationStep(ctx: Context, userId: number) {
    ...
    if (!draft.title) {
      ...
      await ctx.reply('📄 Напиши описание — что именно нужно сделать:');
      return;
    }
    if (!draft.description) {
      draft.description = text;
      pendingTaskCreation.set(userId, draft);
      await ctx.reply('💰 Сколько Макскоинов за выполнение? (напиши число)');
      return;
    }
    if (!draft.reward) {
      const reward = parseInt(text, 10);
      if (isNaN(reward) || reward <= 0) { await ctx.reply('Напиши целое положительное число, например: 10'); return; }
      draft.reward = reward;
      pendingTaskCreation.set(userId, draft);
      const keyboard = new InlineKeyboard()
        .text('1️⃣ Разовое', 'tasktype:once')
        .text('🔄 Повторяющееся', 'tasktype:recurring');
      await ctx.reply('Это разовое задание или повторяющееся?', { reply_markup: keyboard });
    }
  }
  ...
  bot.callbackQuery(/^tasktype:(once|recurring)$/, async (ctx) => {
    ...
    await ctx.editMessageText(
      `✅ Задание создано!\n\n📌 *${task.title}*\n💰 ${task.reward} Макскоинов\n` +
      `${type === 'recurring' ? '🔄 Повторяющееся' : '1️⃣ Разовое'}`,
      { parse_mode: 'Markdown' }
    );
  });
  ...
  bot.callbackQuery('admin:tasks', async (ctx) => {
    ...
    if (!tasks.length) { await ctx.reply('Заданий пока нет.', { reply_markup: keyboard }); return; }
  ...
  bot.callbackQuery('admin:submissions', async (ctx) => {
    ...
    if (!subs.length) { await ctx.reply('Нет заданий на проверке ✅'); return; }
...
import { getPendingSubmissions } from '../../db/tasks';
